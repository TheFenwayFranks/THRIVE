...
export class ProgressStorageService {
  ...
  /**
   * Get today's completed task IDs for UI display
   */
  static async getTodayCompletedTaskIds(): Promise<string[]> {
    try {
      const completedData = await AsyncStorage.getItem(STORAGE_KEYS.COMPLETED_TASKS);
      if (!completedData) {
        return [];
      ...
      // Return array of task IDs completed today
      const todayTasks = completed[today] || [];
      return todayTasks.map((task: any) => task.taskId);
    } catch (error) {
      console.error('❌ Error getting today\'s completed task IDs:', error);
      return [];
    }
  }
